...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/21 09:14:01 by momari            #+#    #+#             */
/*   Updated: 2024/02/21 16:02:19 by momari           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check_all(t_data *data)
{
	...
	if (data->n_colle < 1 || data->n_enemy < 1
		|| data->n_exitm != 1 || data->n_playe != 1)
		ft_handle_error(-4, "Error", data);
}

void	ft_flood_fill(t_data *data, int y, int x)
{
	if (data->map_1[y][x] == '1' || data->map_1[y][x] == 'I')
		return ;
	if (data->map_1[y][x] == 'P')
	{
		data->map_1[y][x] = '1';
